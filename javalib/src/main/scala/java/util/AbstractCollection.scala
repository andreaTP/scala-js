...
abstract class AbstractCollection[E] extends java.util.Collection[E] {
  ...
  @tailrec
  private def iterateAndDo[X](i: Iterator[E], f: E => X, 
      part: SList[X] = SList()): SList[X] = {
    if (i.hasNext) iterateAndDo(i, f, f(i.next()) :: part)
    else part
  }

  @tailrec
  private def iterateApplyAndDo[X](i: Iterator[E], f: (Iterator[E], E) => X, 
      part: SList[X] = SList()): SList[X] = {
    if (i.hasNext) iterateApplyAndDo(i, f, f(i, i.next()) :: part)
    else part
  }
  ...
  def containsAll(c: java.util.Collection[_]): Boolean =
    !iterateAndCheck(c.iterator, !contains(_: Any))
  ...
  def toArray[X](a: Array[X]): Array[X] = {
    val iter = iterator
    for { i <- 0 until size } { a(i) = iter.next.asInstanceOf[X] }
    a
  }

  def toArray(): Array[Any] = {
    val iter = iterator
    val elems = for (i <- 0 until size) yield iter.next()
    elems.toArray[Any]
  }
